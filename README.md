...
- **App:** [uw-compass-tau.vercel.app](https://uw-compass-tau.vercel.app/) — or run locally with `npm run dev`
- **About / how it works:** [/about](https://uw-compass-tau.vercel.app/about) on the deployed app
...
flowchart TB
    ...
    Supabase[("Supabase<br/>gallery_queries · feedback")]
    GalleryAPI["app/api/gallery/route.ts"]
    FeedbackAPI["app/api/feedback/route.ts"]

    Browser -- "POST { input, shareQuery }" --> API
    API --> Embed
    API --> Extract
    Embed --> Rank
    Extract --> Rank
    ...
    API -. "shareQuery=true (async)" .-> Supabase
    Browser -- "GET" --> GalleryAPI --> Supabase
    Browser -- "POST { helpful }" --> FeedbackAPI --> Supabase
```
...
Shared queries and feedback votes are stored in **Supabase** (Postgres). The gallery (`GET /api/gallery`) reads the 10 most recent anonymously shared queries on page load; the feedback endpoint writes one row per vote.
...
app/
  ...
  api/recommend/route.ts  POST endpoint — also writes shared queries to Supabase
  api/gallery/route.ts    GET endpoint — reads recent shared queries from Supabase
  api/feedback/route.ts   POST endpoint — writes feedback votes to Supabase
lib/
  ...
  supabase.ts             Supabase client (server-side only, uses service_role key)
data/
...
Returns `{ queries: string[] }` — the 10 most recent anonymously shared queries from Supabase. Returns `[]` if no queries have been shared yet or if Supabase is not configured.
...
The app deploys directly to Vercel. Resource embeddings are bundled as `data/resources.embedded.json` at build time. Gallery and feedback data are stored in Supabase (Postgres).

1. Push to `main`.
2. Import the repo at [vercel.com/new](https://vercel.com/new) (you need push access).
3. Create a [Supabase](https://supabase.com) project and run the schema below in the SQL editor.
4. Add these environment variables in Vercel under **Settings → Environment Variables**:
...
5. Deploy. Every subsequent push to `main` auto-deploys.

**Supabase schema** (run once in the SQL editor):

```sql
create table gallery_queries (
...
- [x] Feedback buttons ("helpful" / "not helpful") with anonymous logging to Supabase
- [x] Shared query gallery — opt-in anonymous queries stored in Supabase, shown to other students on page load
